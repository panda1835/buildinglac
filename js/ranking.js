...
submit.onclick = function(){
    ...
    let endowment = getInputValue("endowment")
    let salary = getInputValue("salary")
    let sfratio = getInputValue("sfratio")
    let firstgen = getInputValue("firstgen")
    let clubs = getInputValue("clubs")
    let SAT = getInputValue("SAT")
    let name = getInputValue("schoolname");
    rank = 384.67 - 0.001*tuition -0.0000000148*endowment+ 0.0003*salary + 
               67.1/sfratio + 1.27*firstgen/100 - 0.05*clubs - 0.22*SAT
    myFunction(arr, parseInt(rank), name);
}
...
function myFunction(arr,rank,name) {
    ...
    for(i = 0; i < arr.length/3; i++) {
        if(rank>arr[3*(i-1)+1] && rank<=arr[3*i+1]){
            out+='<div><p><li class="ranking-list" style="color: white;">' + 
            rank +"-"+name + '</li></p></div>';
        }
        // out += '<a href="' + arr[3*i+2] + '"><p><li style="margin:auto;padding:15px;">' + 
        // arr[3*i+1] +"-"+arr[3*i] + '</li></p></a><br>';
        out += '<div><a href='+arr[3*i+2]+' style="display: contents"><li class="ranking-list">' + 
        arr[3*i+1] +"-"+arr[3*i] + '</li></a></div>';
    }
    
    if(rank>arr[arr.length-2]){
        out+='<div><p><li class="ranking-list"  style="color: white;">' + 
        (arr[arr.length-2]+1) +"-"+name + '</li></p></div>';
    }
    document.getElementById("overall_ranking").innerHTML = out;
    // if(rank>0){
        document.getElementById("result").innerHTML = name+ " predicted ranking is "+rank;
    // }
    // if(rank<=0){
    //     document.getElementById("result").innerHTML = name+ " predicted ranking is "+0;
    // }
